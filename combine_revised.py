...
# ===================================================================
program = "Customized Split+federated Learning with ResNet18 on HAM10000"
...
class ResNet18_server_side(nn.Module):
    def __init__(self, block, num_layers, classes):
        ...
        self.averagePool = nn.AvgPool2d(kernel_size=2, stride=1)  # change from 7 to 2
    ...
    def forward(self, x):
        ...
        x7 = F.avg_pool2d(x6, 2)  # change from 7 to 2
...
# =============================================================================
#                    Model definition: ResNet18 Full
# =============================================================================
...
net_glob_full = ResNet18(Baseblock, [2, 2, 2, 2], 7)  # Class labels for HAM10000 = 7

if torch.cuda.device_count() > 1:
    print("We use", torch.cuda.device_count(), "GPUs")
    net_glob_full = nn.DataParallel(net_glob_full)  # to use the multiple GPUs

net_glob_full.to(device)
print(net_glob_full)
...
def train_server(fx_client, y, l_epoch_count, l_epoch, idx, len_batch):
    ...
    global s_train_acc, s_train_loss
    ...
    if count1 == len_batch:
        ...
        s_train_acc += acc_avg_train
        s_train_loss += loss_avg_train
...
def evaluate_server(fx_client, y, idx, len_batch, ell):
    ...
    with torch.no_grad():
        ...
        if count2 == len_batch:
            ...
            prGreen('Client{} Test =>                   \tAcc: {:.3f} \tLoss: {:.4f}'.format(idx, acc_avg_test,
                                                                                             loss_avg_test))
            s_test_acc += acc_avg_test
            s_test_loss += loss_avg_test
    ...
    return
# ==============================================================================================================
#                                       Server-side full training Program
# ==============================================================================================================
class Server_full(object):
    def __init__(self, net_server_model, lr, device, dataset_train=None, dataset_test=None,idxs=None,idxs_test=None):
        self.device = device
        self.lr = lr
        self.local_ep = 1
        # self.selected_clients = []
        #self.ldr_train = DataLoader(dataset_train, batch_size=256, shuffle=True)
        #self.ldr_test = DataLoader(dataset_test, batch_size=256, shuffle=True)
    ...
    def evaluate(self, model, device, criterion):
        epoch_loss = 0
        epoch_acc = 0
        model.eval()
        ell = len(self.ldr_test)
...
optimizer_full = torch.optim.Adam(net_glob_full.parameters(), lr=lr)
criterion_full = nn.CrossEntropyLoss()
s_train_acc = 0
s_train_loss = 0
s_test_acc = 0
...
for iter in range(epochs):
    ...
    full = Server_full(net_glob_full,lr, device, dataset_train=dataset_train, dataset_test=dataset_test,idxs=dict_users[1], idxs_test=dict_users_test[1])
    train_loss, train_acc = full.train(net_glob_full, device, optimizer_full, criterion_full)
    test_loss, test_acc = full.evaluate(net_glob_full, device, criterion_full)
    s_train_acc  += train_acc
    s_train_loss += train_loss
    s_test_acc += test_acc
    s_test_loss += test_loss
    print(f'Server Train Full => Epoch: {iter} \t Acc: {train_acc :05.2f}% \t Loss: {train_loss:.3f}')
    print(f'Server Test Full  =>               \t Acc: {test_acc :05.2f}% \t Loss: {test_loss:.3f}')

    #for idx in idxs_users:
    local = Client(net_glob_client, 0, lr, device, dataset_train=dataset_train, dataset_test=dataset_test,
                       idxs=dict_users[0], idxs_test=dict_users_test[0])
        # Training ------------------
    ...
    print("-----------------------------------------------------------")
    print("------ FedServer: Federation process at Client-Side, Server Side, and Server Full Model updates ------- ")
    print("-----------------------------------------------------------")
    w_full = net_glob_full.state_dict()
    w_locals_client.append(w_full)
    w_glob_client = FedAvg(w_locals_client)

    w_glob_server_up = []
    w_glob_server_up.append(copy.deepcopy(w_glob_server))
    w_glob_server_up.append(w_full)
    w_glob_server_fed = FedAvg(w_glob_server_up)

    global_w = w_glob_client.copy()
    global_w.update(w_glob_server)

    # Update client-side, server-sde, server full traning global model
    net_glob_client.load_state_dict(w_glob_client)
    net_glob_server.load_state_dict(w_glob_server_fed)
    net_glob_full.load_state_dict(global_w)

    print(' Train: Round {:3d}, Avg Accuracy {:.3f} | Avg Loss {:.3f}'.format(iter, s_train_acc/2,
                                                                              s_train_loss/2))
    print(' Test: Round {:3d}, Avg Accuracy {:.3f} | Avg Loss {:.3f}'.format(iter, s_test_acc/2,
                                                                             s_test_loss/2))
